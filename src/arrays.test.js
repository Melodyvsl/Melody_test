...
describe('getNamesOfPeopleOver', () => {
  const getNamesOfPeopleOver = limit => {
    ...
    return people.filter(person => person.age > limit).map(e => e.name);
  };
...
describe('getInterests', () => {
  const getInterests = () => {
    // TODO: Complete this function to allow the test to pass
    let res = [];
    people.map(interest => interest.interests.map(e => res.push(e)));
...
describe('getNamesAndInterests', () => {
  const getNamesAndInterests = () => {
    // TODO: Complete this function to allow the test to pass
    let peopleInterests = {};
    people.map(e => (peopleInterests[e.name] = e.interests));
